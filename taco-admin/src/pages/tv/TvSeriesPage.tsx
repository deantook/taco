...
export function TvSeriesPage() {
  ...
  const [payload, setPayload] = useState<PagePayload<TvSeries> | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setListError(null)
    try {
      const data = await tvApi.fetchTvSeriesPage(current, size)
      setPayload(data)
    } catch (e) {
  ...
  function openEdit(row: TvSeries) {
    if (row.id == null) {
      return
    }
    setMode('edit')
    ...
    setSeasonCount(row.seasonCount != null ? String(row.seasonCount) : '')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === 'create') {
        delete body.id
        await tvApi.createTvSeries(body)
      } else if (editId != null) {
        await tvApi.updateTvSeries(editId, body)
      }
      closeModal()
      await load()
    } catch (err) {
  ...
  async function handleDelete(id: number) {
    if (!window.confirm('确定删除该剧集？')) {
      return
    }
    try {
      await tvApi.deleteTvSeries(id)
      await load()
    } catch (e) {
                  ...
                  payload.records.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap px-4 py-3 text-neutral-600 dark:text-neutral-300">
                        {row.seasonCount ?? '—'} / {row.episodeCount ?? '—'}
                      </td>
                      <td className="whitespace-nowrap px-4 py-3 text-neutral-600 dark:text-neutral-300">
